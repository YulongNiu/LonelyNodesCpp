...
$X$: $x \in X$ are searched vertices. Maximal cliques that contain $\forall x \in X$ have been searched.
...
Check if $X = \emptyset$ in $s_{stop}$, which means there is no vertices in $X$ adjacent to vertices in $R$. If so, then $R$ is a new maximal clique.
...
`LOVO` is designed based on the fact that: $\forall p \in P$ are searched in status $s$, and cliques in $s_{stop}$ is checked by $\Gamma(x)$. So for a given $x_n \in X$ if $P$ contains only $P \cap \Gamma(x_n)$, the status can be skipped.

The skippable status is described as:

\begin{equation*}
\begin{split}
s_{skip} &: \{x \in X\}, \{  r \in R \}, \{ P \cap \Gamma(x_n | x_n \in X) \} \\
\end{split}
\end{equation*}

$P \cap \Gamma(x_n | x_n \in X)$ is equal to the number of skipped iterations.

In `LOVO`, $x_n$ is chosen to maximize $|N(x_i|x_i \in X)|$:
...
* step1: select $x_n$ to maximize $|N(x_i|x_i \in X)|$,

*Case1*: $P \setminus N(x_n) \neq \emptyset$, randomly select $p_1$ from $P \setminus N(x_n)$. In this way, $p \in  N(x_n)$ are always searched in last rank.

*Case2*: $P \setminus N(x_n) = \emptyset$, skip current status, then backtrack to the second latest status.

The status $s_{new}$ has the same format as \@ref(eq:1). The status $s$ is updated as \@ref(eq:2).

* step2: Use the same vertex selection and ordering method for $s_{new}$. Note that $x_n^{\prime}$ in $s_{new}$ may be different from the $x_n$ in $s$. It means $x_n$ is always chosen according to the current status $s$ and make the $|N(x_n)|$ local optimal. Repeat `step1` until $P = \emptyset$, then check whether current $R$ is a maximal clique.
...
Different from Bron-Kerbosch's algorithm, `LOVO` "ordered" vertices in $P$ that roughly split $P$ into two groups $N(x_n)$ and $P \setminus N(x_n)$. Vertex $p$ for next search was randomly selected from $P \setminus N(x_n)$.

`LOVO` always checked whether the union of $R$ and $P$ is part of searched maximal cliques, so invalid iterations could be avoid. *Case3* in `step1` above described the condition, $R \subset \Gamma(x_n)$ and $P \setminus N(x_n) = \emptyset$ that equals to $(R \cup P) \subset \Gamma(x_n | x_n \in X)$. Besides, maximized $|N(x_n)|$ guaranteed that largest possible searched maximal cliques or parts were checked in high priority. 

It is possible that a chosen vertex $p_x$ from $P \setminus N(x_n)$ actually formed a searched maximal cliques. $p_x$ was omitted just because its corresponding $N(x_m)$ is not maximal. Any $p_x$ like below should not be searched:
...
Interestingly, as described in Bron-Kerbosch's algorithm, for a given status $s$, $\forall p \in P$ $X \cap \Gamma(p) = X \mathrm{or} \: \emptyset$. It means each vertex in $X$ is adjacent to every or none of vertex in in $P$. We denote $X \cap \Gamma(p)$ as $N(p)$.
...
For $s_{next}$ above, if $\Gamma(p_1) \cap x_1 = \emptyset$, then $N(p) = \emptyset$. Or if $\Gamma(p_1) \cap x_1 = x_1$, it means $x_1$ is adjacent to $p_1$, then $N(p) = x_1$. For the $s_{update}$ above, $N(p) = \emptyset$, the same as $s$.

If the assertion above holds for status:
...
\begin{equation*}
\begin{split}
s^{\prime}_{next} &: \{ \Gamma(p_1) \cap \dots \cap \Gamma(p_n) \cap \Gamma(p_{n+1}) \cap X \}, \{r \in R, p_1, \dots, p_n, p_{n+1} \}, \{ \Gamma(p_{n+1}) \cap \{ P \setminus \{ p_1, \dots, p_n \} \} \} \\
\end{split}
\end{equation*}
...
\begin{equation*}
\begin{split}
s^{\prime}_{update} &: \{ \Gamma(p_1) \cap \dots \cap \Gamma(p_n) \cap X , p_{n+1}\}, \{r \in R, p_1, \dots, p_n \}, \{ P \setminus \{ p_1, \dots, p_n, p_{n+1}\} \} \\
\end{split}
\end{equation*}

For $s^{\prime}_{next}$ above, if $X^{\prime}_{next} = \{ \Gamma(p_1) \cap \dots \cap \Gamma(p_n) \cap \Gamma(p_{n+1}) \cap X \}$ is not empty, 
