...
The status $s_{new}$ has the same format as \@ref(eq:1). Because $x_n \notin  \Gamma(p_1) \cap X$, so the mode of $X$ is reduced in $s_{new}$. The status $s$ is updated as \@ref(eq:2).
...
\begin{equation*}
\begin{split}
\{ \Gamma(p_x) \cap P, p_x \} \subset \Gamma(x_m | x_m \in X) \\
\end{split}
\end{equation*}
...
Because $p_x$ and $x_m$ are adjacent, $x_m \in \Gamma(p_x) \cap X$. 
$s_{new}$ would be skipped before next search.
