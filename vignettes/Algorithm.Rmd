...
The skippable status is described as, $\exists x_n$:
...
* step2: use the same vertex selection and ordering method for $s_{new}$. Note that $x_n^{\prime}$ in $s_{update}$ may be different from the $x_n$ in $s$. It means $x_n$ is always chosen according to the current status $s$ and make the $|N(x_n)|$ local optimal. Repeat `step1` until $P = \emptyset$, then check whether current $R$ is a maximal clique.
...
It is possible that a chosen vertex $p_x$ from $P \setminus N(x_n)$ actually formed a searched maximal cliques. $p_x$ was omitted just because its corresponding $N(x_m)$ is not maximal. Any $p_x$ like below should be omitted:
...
Because $p_x$ and $x_m$ are adjacent, $x_m \in \Gamma(p_x) \cap X$. Then $x_m$ would be chosen in $s_{new}$, so $s_{new}$ would be skipped before next search.
